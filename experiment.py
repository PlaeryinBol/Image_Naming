...
class Experiment(object):
    '''
    A class for creating an experiment of the show, attend and tell network. The experiment stores
    the main architecture of the network and its train and validate functions can be used
    to run the experiment over required number of epochs.
    Additonally, it is possible to store/load the state of the model in the experiment between epochs.
    ...
    '''
    def __init__(self, start_epoch, encoder, decoder, optimizer, loss_func, train_loader,
                 val_loader, word_dict, alpha_c, log_file, log_interval, device):
    ...
    def save(self, epoch, model):
        ...
        '''
        torch.save({'epoch': epoch + 1, 'state_dict': self.decoder.state_dict(),
    ...
    def train(self, epoch):
        ...
        print(f'Start train from {epoch} epoch')
        self.encoder.eval()
        self.decoder.train()
        ...
        for batch_idx, (imgs, captions) in enumerate(self.train_loader):
            ...
            preds, alphas = self.decoder(img_features, captions)
            targets = captions[:, 1:]
            ...
            # and using backpropogation update the decoder based on this loss
            ...
            if batch_idx % self.log_interval == 0:
                self.log_file.write('Train Batch: [{0}/{1}]\t'
                                    ...
                                    .format(batch_idx, len(self.train_loader), loss=losses, top1=top1, top5=top5))
    ...
    def validate(self, epoch):
        ...
        self.log_file.write(f'Start validation of {epoch} epoch')
            ...
            for batch_idx, (imgs, captions, all_captions) in enumerate(self.val_loader):
                ...
                if batch_idx % self.log_interval == 0:
                    self.log_file.write('Validation Batch: [{0}/{1}]\t'
                                        ...
                                        'Top 1 Accuracy {top1.val:.3f} ({top1.avg:.3f})\t'
                                        'Top 5 Accuracy {top5.val:.3f} ({top5.avg:.3f})\n'
                                        .format(batch_idx, len(self.val_loader), loss=losses, top1=top1, top5=top5))
                    print('Val Batch: [{0}/{1}]\t'
                          'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                          'Top 1 Accuracy {top1.val:.3f} ({top1.avg:.3f})\t'
                          'Top 5 Accuracy {top5.val:.3f} ({top5.avg:.3f})'
